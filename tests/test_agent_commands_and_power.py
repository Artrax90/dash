...
@pytest.mark.anyio
async def test_bulk_wake_sets_booting_and_power_grace_without_false_offline_alert():
    """
    Verify that bulk WAKE in Web UI:
    1. Sets PowerStatus.BOOTING (never premature PowerStatus.ON).
    2. Sets power grace period (120-180s) in scheduler_service so watchdog ignores boot phase.
    3. Resets ping failure counters.
    4. Does NOT trigger false 'connection lost' offline alerts in Telegram.
    """
    from backend.app.models.device import Device, PowerStatus
    from backend.app.api.v1.devices import execute_bulk_operation
    ...
    test_id = "PC-BULK-WAKE-TEST-01"
    async with AsyncSessionLocal() as db:
        try:
            dev = Device(
                id=test_id,
                name="Bulk Wake Test Machine",
                hostname="HOST-BWAKE-01",
                ip_address="172.16.42.99",
                mac_address="00:11:22:33:44:55",
                power_status=PowerStatus.OFF,
            ...
            # Set consecutive ping failures to simulate an offline PC
            scheduler_service._consecutive_ping_failures[test_id.upper()] = 25

            payload = BulkOperationRequestSchema(
                deviceIds=[test_id],
                action="WAKE"
            )

            mock_req = MagicMock()
            mock_req.headers = {"X-User-Role": "Администратор парка"}

            with patch("backend.app.services.wol_service.wol_service.send_magic_packet", return_value=True):
                await execute_bulk_operation(payload, mock_req, db)

            # Check DB status
            updated_dev = await db.get(Device, test_id)
            assert updated_dev.power_status == PowerStatus.BOOTING, f"Device power_status should be BOOTING, got {updated_dev.power_status}"

            # Check scheduler grace period and ping failure reset
            grace_until = scheduler_service._power_action_grace_until.get(test_id.upper(), 0)
            assert grace_until > time.time() + 60, "Scheduler grace period should be set to >= 60s"
            assert scheduler_service._consecutive_ping_failures.get(test_id.upper()) == 0, "Ping failures should be reset to 0"

        finally:
            await db.execute(delete(Device).where(Device.id == test_id))
            await db.commit()

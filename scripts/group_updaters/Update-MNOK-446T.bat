...
echo    Токен:  wm_tok_c52d8745c47fead0
echo    ПК:     0 шт.
...
echo   [1] Удаленное обновление всех 0 станций кабинета (по сети)
...
if "%MODE%"=="2" (
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%Update-MNOK-446T.ps1" -LocalInstall
) else if "%MODE%"=="3" (
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%Update-MNOK-446T.ps1" -PingOnly
) else (
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%Update-MNOK-446T.ps1"
)
